jQuery(function ($) {
    $(document).ready(function () {
                        ...
                        `<div class="question-wrap question-scale nps-question-scale question-new" data-id="${id}">
                                ...
                                <div class="box-shadow question-settings">
                                            ...
                                            <div class="options-item">
                                                ...
                                                <div class="option-value">
                                                    <select name="scaleAmount_${id}" class="customselect scale-amount">
                                                        ...
                                                        <option value="8">8</option>
                                                        <option value="9">9</option>
                                                        <option selected value="11">10</option>
                                                        <option value="2">да/нет</option>
                                                    </select>
                                                </div>
                                            </div>
                                    ...
                                    <div class="switch-row nps-option-row">
                                        ...
                                        <label class="switch">
                                            <input type="checkbox" class="addNpsOption" name="npsOptionLabels_${id}" id="npsOptionLabels_${id}">
                                            <span class="slider round"></span>
                                        </label>
                                    </div>
                                </div>
        ...
        function setDiapasonMax(question) {
            ...
            if (question[0].classList.contains('nps-question-scale')) {
                max = parseInt(question.find('.scale-amount').val() - 1);
        ...
        function createNpsOptionBlock(question, npsOptionName) {
            let npsOptionBlock =
                    ...
                    <div class="nps-option">
                        <div class="scale-property-row">
                            ...
                            <div class="nps-diapason-start nps-diapason-box">
                                <span>От</span>   
                                <div class="option-value">
                                    <select name="npsDiapasoneStart_${npsOptionName}_1" class="customselect">
                            ...
                            <div class="nps-diapason-end nps-diapason-box">
                                <span>До</span>   
                                <div class="option-value">
                                    <select name="npsDiapasoneEnd_${npsOptionName}_1" class="customselect">
                            ...
                            <div class="nps-action-box"> 
                                <div class="option-value">
                                    <select name="npsAction_${npsOptionName}_1" class="customselect select-nps-action" data-required>
                                        <option selected value="Выбрать действие">Выбрать действие</option>
                                        <option value="Добавить открытый вопрос">Добавить открытый вопрос</option>
        ...
        function createNpsOption(question, npsOptionName, npsNameIndex, npsOptionsCount, newNpsOptionsCount) {
                ...
                `<div class="nps-option">
                        <div class="scale-property-row">
                            ...
                            <div class="nps-diapason-start nps-diapason-box">
                                <span>От</span>   
                                <div class="option-value">
                                    <select name="npsDiapasoneStart_${npsOptionName}_${npsNameIndex}" class="customselect">
                                        <option selected value="0">0</option>
                                        <option value="1">1</option>
                                        <option value="2">2</option>
                                        <option value="3">3</option>
                            ...
                            <div class="nps-diapason-end nps-diapason-box">
                                <span>До</span>   
                                <div class="option-value">
                                    <select name="npsDiapasoneEnd_${npsOptionName}_${npsNameIndex}" class="customselect">
                                        <option value="0">0</option>
                                        <option value="1">1</option>
                                        <option value="2">2</option>
                                        <option value="3">3</option>
        ...
        $('.content-wrap').on('change', '.question-scale .addNpsOption', function () {
            ...
            if ($(this).is(':checked')) {
                createNpsOptionBlock(question, npsOptionName);
            } else {
        ...
        $('.content-wrap').on('click', '.question-scale .add-new-option', function () {
            ...
            createNpsOption(question, npsOptionName, npsNameIndex, npsOptionsCount, newNpsOptionsCount);
        });

        function checkNpsDiapason(question, currentSelect) {
            let npsOptions = $(currentSelect).parents('.nps-options-box').find('.nps-option');
            const currentAction = $(currentSelect).parents('.nps-options-box').find('.nps-action-box').val();

            let firstOptionSelect = $(npsOptions[0]).find('.nps-diapason-box select');
            let inCorrectOptionsList = [];
            const v1 = $(firstOptionSelect[0]).val();
            const v2 = $(firstOptionSelect[1]).val();
            const selectedAreas = [];

            if (v1 > v2) {
                $(npsOptions[0]).css('background', 'red')
            } else {
                $(npsOptions[0]).css('background', 'white')
            ...
            for (let i = 0; i < npsOptions.length; i++) {
                ...
                let currentFromVal = $(currentOptionSelect[0]).val() * 1;
                let currenToVal = $(currentOptionSelect[1]).val() * 1;


                console.log(selectedAreas, currenToVal, currentFromVal);
                if (currentFromVal > currenToVal) {
                    $(npsOptions[i]).css('background', 'red');
                } else if (i != 0) {
                    let isValid = true;
                    selectedAreas.forEach(v => {
                        if (v[0] >= currenToVal || v[1] >= currentFromVal) {
                            isValid = false;
                        }
                    });
                    if (isValid) {
                        selectedAreas.push([currentFromVal, currenToVal])
                        $(npsOptions[i]).css('background', 'white');
                    } else {
        ...
        $('.content-wrap').on('change', '.question-scale .select-nps-action', function () {
            ...
            /** Validate select */
            if (npsAction != 'Выбрать действие') {
                $(this).removeAttr('data-required', '');
                $(this).parents('.select').find('.select-styled').removeClass('has-error');
            } else {
                $(this).attr('data-required', '');
            }
            if (npsAction == 'Добавить открытый вопрос') {
                // console.log($(this).parents('.nps-option').find('.nps-diapason-start select'))
